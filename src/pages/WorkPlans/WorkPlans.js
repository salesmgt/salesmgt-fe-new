...
function WorkPlans() {
    ...
    async function callAPIRemoveUpdate(e) {
        ...
        e.data.forEach(item => {
            ...
            if (recurrenceExp)
                recurrenceExp = recurrenceExp + "," + recurrence
            else
                recurrenceExp = recurrence
        });
    ...
    async function callAPI(filter2) {
        const convert = {
            ...filter2,
            currentDate: formatISO9075(filter2.currentDate, { representation: 'date' })
        }
        WorkPlansServices.callAPI(convert).then(res => {
            res.data?.forEach(element => {
                if (!element.recurrenceID)
                    delete element.recurrenceID
                element = {
                    ...element,
    ...
    async function callAPITree(key) {
        ...
        WorkPlansServices.callAPITree(convert).then(data => {
